...
<body class="bg-gradient-to-br from-orange-400 via-yellow-500 to-orange-600 font-roboto min-h-screen">
    <div class="min-h-screen flex flex-col justify-center items-center">
        <div class="bg-white p-20 rounded-md">
            ...
            @endif
            <h1 class="text-3xl font-semibold text-center mb-5">Register Pengguna</h1>
            <form method="post" action="{{ route('validate-register') }}" class="max-w-sm mx-auto" name="register_form"
                ...
                <x-forms.image-input>
                    <x-slot:judul>Foto Profil</x-slot:judul>
                    <x-slot:name>photo</x-slot:name>
                </x-forms.image-input>

                <div class="w-full flex justify-center">
                    <button type="submit"
                        class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Daftar</button>
                </div>
            </form>
            <p class="my-5 text-center">Sudah ada akun? <a href="{{ route('login') }}" class="text-blue-700">klik
                    disini</a></p>
        </div>
