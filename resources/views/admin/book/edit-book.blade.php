<x-admin.layout>
    ...
    <div class="min-h-screen flex flex-col justify-center items-center px-24">
        ...
        <form method="POST" action="{{ route('books.update', $book->id) }}"
            class="w-full mx-auto grid grid-cols-1 md:grid-cols-2 gap-10" name="book_update" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <h1 class="text-bold text-4xl">Ubah Buku</h1>
            ...
            <x-forms.normal-input>
                ...
                <x-slot:value>{{ $book->title }}</x-slot:value>
            </x-forms.normal-input>
            <x-forms.normal-input>
                ...
                <x-slot:pattern>.*</x-slot:pattern>
                <x-slot:value>{{ $book->author }}</x-slot:value>
            </x-forms.normal-input>
            <x-forms.normal-input>
                ...
                <x-slot:pattern>.*</x-slot:pattern>
                <x-slot:value>{{ $book->publisher }}</x-slot:value>
            </x-forms.normal-input>
            <x-forms.normal-input>
                ...
                <x-slot:pattern>.*</x-slot:pattern>
                <x-slot:value>{{ $book->release_date }}</x-slot:value>
            </x-forms.normal-input>
            ...
            <div class="col-span-1 md:col-span-2 w-full gap-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach ($kategori as $kat)
                            <x-forms.check-box>
                                ...
                                <x-slot:checked>
                                    {{ in_array($kat->id, old('categories', $selectedCategories) ?? []) ? 'checked' : '' }}
                                </x-slot:checked>
                            </x-forms.check-box>
                        @endforeach
                    </div>
                ...
                <div class="h-fit mb-5">
                    <x-forms.text-area-input>
                        ...
                        <x-slot:value>{{ $book->description }}</x-slot:value>
                    </x-forms.text-area-input>
                </div>
                ...
                <x-forms.image-input>
                    ...
                    <x-slot:value>{{ $book->cover_photo }}</x-slot:value>
                </x-forms.image-input>
            </div>
        </form>
    </div>
